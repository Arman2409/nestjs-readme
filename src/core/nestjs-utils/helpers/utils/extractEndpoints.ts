...
const extractEndpoints = (content: string) => {
    ...
    while ((match = methodRegex.exec(content)) !== null) {
        ...
        if (methodsDecorators.includes(method)) {
            ...
            const sourceMath = (newMatch as any)[0].match(regexForLine);

            const name = sourceMath[2];   // Parameter name (e.g., updateProductDto)
            const type = sourceMath[3];   // Parameter type (e.g., CreateProductDto)
